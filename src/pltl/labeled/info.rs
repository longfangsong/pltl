...
impl LabeledPLTL {
    ...
    pub fn u_v_subformulas(&self) -> (Set<Self>, Set<Self>) {
        match self {
            ...
            LabeledPLTL::BinaryTemporal { lhs, rhs, .. } => {
                let (u, v) = lhs.u_v_subformulas();
                let (u, v) = rhs.u_v_subformulas();
                (u, v)
            }
        }
    }
...
mod tests {
    ...
    //     let pltl = LabeledPLTL::new_binary(BinaryOp::Since, Box::new(LabeledPLTL::Atom(0)), Box::new(LabeledPLTL::Atom(1)));
    //     let weaken_condition = pltl.weaken_condition();
    //     assert_eq!(weaken_condition, LabeledPLTL::new_binary(BinaryOp::Or, Box::new(LabeledPLTL::Atom(0)), Box::new(LabeledPLTL::Atom(1))));
    // }
}
